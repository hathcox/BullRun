...
namespace BullRun.Tests.Core
{
    ...
    public class GameEventsTests
    {
        ...
        // --- RunEndedEvent Tests (Story 4.4 Task 4, consolidated Story 6.5 Task 6) ---
        ...
        public void RunEndedEvent_StoresAllFields()
        {
            ...
            Assert.IsFalse(evt.IsVictory);
            Assert.AreEqual(0, evt.ReputationEarned);
            Assert.AreEqual(3, evt.ItemsCollected);
            Assert.AreEqual(3000f, evt.PeakCash, 0.01f);
            Assert.AreEqual(800f, evt.BestRoundProfit, 0.01f);
        }
