...
/// and the GameFeelManager MonoBehaviour. Called from GameRunner.Start().
/// </summary>
public static class GameFeelSetup
{
    public static void Execute()
    {
        ...
        // Sizes are in canvas-space pixels (ref 1920x1080). Keep small for sparks/confetti.
        // Trade particles: positioned at chart head at emit time; short life + low speed = contained pop
        //                                                    name                   anchor          color                size  life  speed
        var tradeParticles = CreateParticleEmitter("TradeParticles", canvasGo.transform,
            new Vector2(0f, 0f), ColorPalette.Green,          20f,  0.7f, 8f);

        var celebrationParticles = CreateParticleEmitter("CelebrationParticles", canvasGo.transform,
            new Vector2(0f, 400f), ColorPalette.Gold,         4f,   1.5f, 80f);

        var currencyParticles = CreateParticleEmitter("CurrencyParticles", canvasGo.transform,
            new Vector2(-600f, -400f), ColorPalette.Gold,     3f,   1.2f, 50f);

        var fullScreenParticles = CreateParticleEmitter("FullScreenParticles", canvasGo.transform,
            new Vector2(0f, 0f), ColorPalette.Red,            6f,   1.5f, 100f);
        ...
        // ── 5. MMCameraShaker on Main Camera ─────────────────────────────
        var cam = Camera.main;
        ...
        // ── 6. GameFeelManager MonoBehaviour ─────────────────────────────
        ...
        Debug.Log("[GameFeel] Setup complete: canvas(sort=15), 5 flash overlays, 4 particle emitters, camera shaker, floating pool(8)");
        #endif
    }
